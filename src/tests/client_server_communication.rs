...
mod client_communication {
    ...
    fn test_message_from_client_to_server() {
        ...
        assert!(matches!(first_response, SimControllerResponseWrapper::Event(SimControllerEvent::PacketSent{..})));
        
        let second_response = client_response_channel.recv().unwrap();
        println!("{:?}", second_response);
        
        assert!(matches!(second_response, SimControllerResponseWrapper::Event(SimControllerEvent::MessageSent(_,_,_))), "Expected message sent");

        let third_response = client_response_channel.recv().unwrap();
        println!("{:?}", third_response);
        assert!(matches!(third_response, SimControllerResponseWrapper::Event(SimControllerEvent::PacketReceived(_))), "Expected packet received");

        let fourth_response = client_response_channel.recv().unwrap();
        println!("{:?}", fourth_response);
        assert!(matches!(fourth_response, SimControllerResponseWrapper::Message(SimControllerMessage::MessageReceived(_,_,_))), "Expected message received");
    }
    ...
    fn test_known_servers() {
        ...
        let first_packet = controller_response_channel.recv().unwrap();
        assert!(matches!(first_packet, SimControllerResponseWrapper::Event(SimControllerEvent::PacketSent{packet_type, ..}) if packet_type == "KnownServers"),"Expected known servers request sent");
        // Second packet is KnownServers response
        let second_packet = controller_response_channel.recv().unwrap();
        assert!(matches!(second_packet, SimControllerResponseWrapper::Message(SimControllerMessage::KnownServers(_))),"Expected known servers response");
    ...
    fn test_registered_servers() {
        ...
        let first_packet = controller_response_channel.recv().unwrap();
        assert!(matches!(first_packet, SimControllerResponseWrapper::Event(SimControllerEvent::PacketSent{packet_type, ..}) if packet_type == "RegisteredServers"),"Expected registered servers request sent");
        // Second packet is RegisteredServers response
        let second_packet = controller_response_channel.recv().unwrap();
        assert!(matches!(second_packet, SimControllerResponseWrapper::Message(SimControllerMessage::RegisteredServersResponse(_))),"Expected registered servers response");
