...
fn validate_config(config: &Config) {
    ...
    for client in &config.client {
        for connected_node in &client.connected_drone_ids {
            ...
            for server in &config.server {
                assert!(server.id != *connected_node, "Client {} cannot be connected to server {}", client.id, server.id);
            }

            for other_client in &config.client {
                assert!(other_client.id != *connected_node, "Client {} cannot be connected to client {}", client.id, other_client.id);
            }
    ...
    for server in &config.server {
        for connected_node in &server.connected_drone_ids {
            ...
            for client in &config.client {
                assert!(client.id != *connected_node, "Server {} cannot be connected to client {}", server.id, client.id);
            }

            for other_server in &config.server {
                assert!(other_server.id != *connected_node, "Server {} cannot be connected to server {}", server.id, other_server.id);
            }
